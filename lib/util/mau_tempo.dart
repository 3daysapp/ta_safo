    ...
    articleRegex.allMatches(html).forEach((RegExpMatch match) {
      ...
      pRegex.allMatches(match[1]).forEach((RegExpMatch match) {
        if (match[1].contains(areaRegex)) {
          if (backup != null) {
            FirebaseCrashlytics.instance.log('Erro antes da área: $backup');
            backup = null;
          }
          ...
          if (area != null) {
            ...
            FirebaseCrashlytics.instance.log('Número sem área: ${match[1]}');
          }
        } else {
          ...
          if (backup != null) {
            ...
            if (texto.isNotEmpty) {
              FirebaseCrashlytics.instance.log('Não processado: $texto');
              backup.texto = texto;
              list.add(backup);
              backup = null;
            }
